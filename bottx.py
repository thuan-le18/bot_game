...
async def choose_taixiu(message: types.Message):
    user_id = str(message.from_user.id)
    log_action(user_id, "Chọn cược", message.text)
...
async def choose_combo_number(message: types.Message):
    ...
    log_action(user_id, "Chọn số", f"{bet_type} - Số {chosen_number}")
    await message.answer(
...
async def play_taixiu(message: types.Message):
    ...
    log_action(user_id, "Kết quả", f"Xúc xắc: {dice_values}, Tổng: {total}, Kết quả: {result}, {outcome_text}")
    # Gửi kết quả
